...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, goal_dim, size):
        ...
        self.goal_buf = np.zeros(core.combined_shape(size, goal_dim), dtype=np.float32)
        self.rew_buf = np.zeros(size, dtype=np.float32)
        self.done_buf = np.zeros(size, dtype=np.float32)
        self.ptr, self.size, self.max_size = 0, 0, size

    def store(self, obs, act, goal, rew, next_obs, done):
        self.obs_buf[self.ptr] = obs.shape[0]
        self.obs2_buf[self.ptr] = next_obs.shape[0]
...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
         ...
         polyak=0.995, pi_lr=1e-3, q_lr=1e-3, batch_size=100, start_steps=250,
         update_after=150, update_every=50, act_noise=0.1, num_test_episodes=10,
    ...
    goal_dim = env.observation_space['desired_goal'].shape
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            q_pi_targ = ac_targ.q(o2, ac_targ.pi(o2,g), g)
    ...
    def compute_loss_pi(data):
        o, g = data['obs'], data['goal']
        q_pi = ac.q(o, ac.pi(o,g), g)
        return -q_pi.mean()
    ...
    def get_action(o, g, noise_scale):
        a = ac.act(torch.as_tensor(o, dtype=torch.float32), torch.as_tensor (g, dtype=torch.float32))
    ...
    def test_agent():
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                o, r, d, _ = test_env.step(get_action(o, g, 0))
                # r = calc_reward(o['achieved_goal'], o['desired_goal'])
                o = o['observation']
                ep_ret += r
                ep_len += 1
